...
os.system('mv *.txt ./save/')

dat0=np.loadtxt('./save/'+'pericar__paramslists.txt')
if(dat0.shape[0]>4):
    np.savetxt('./save/'+'pericar__paramslists.txt',dat0[3:])
...
paramfilenames = ['pericar__paramslists.txt','pericar_paramslists.txt']
...
for filename,pfilename in zip(filenames,paramfilenames):
    ...
    for val,params in zip(df.columns[1:],paramslists):
        ...
        # plt.savefig('./save/'+filename[:7]+'_'+val+'_lml_out.png')
        # plt.close()
        ...
        plt.savefig('./save/'+val+'_gpm_out.png')
        plt.close()
